...
import {IYieldOft, IL1OftRouter, OftYieldConfig} from "../interfaces/IYieldOft.sol";

contract YieldOftBridgeAdapter is IBridgeAdapter, BaseAdapter {
    ...
    mapping(address bridgeToken => OftYieldConfig) public oftLookup;
    mapping(address bridgeToken => address l1Router) public l1Router;
    ...
    ) public onlyAdmin onlyValidLzAdapter(dstChainId, dstLzId, dstDecimals, dstBridgeAdapter) {
     ...
     * @param _config The oft config
     */
    // todo permission here
    function permissionYieldOft(address _bridgeToken, OftYieldConfig calldata _config) external onlyAdmin {
        if (IOFT(_config.oft).token() != _bridgeToken) revert InvalidOft();
        if (!_config.permissioned) revert InvalidOft();
        oftLookup[_bridgeToken] = _config;
        emit OftPermissioned(_bridgeToken, _config.oft);
    }
     ...
     */
    function disallowYieldOft(address _bridgeToken) external onlyAdmin {
     ...
     */
    function bridge(BridgeCall calldata bridgeCall)
        public
        payable
        onlyUtb
        remotePrecision(bridgeCall.dstChainId, bridgeCall.postBridge.swapParams)
    {
        ...
        uint256 sentNative = _sendOft(bridgeToken, oft, sendParam, bridgeCall.refund);
        ...
        if (bridgeToken != address(0)) {
            // convert shares back to assets for dust refund
            uint256 dust = bridgeCall.amount - IYieldOft(bridgeToken).sharesToAssets(sendParam.amountLD);
     ...
     */
    function _sendOft(address bridgeToken, address oft, SendParam memory sendParam, address refund)
        internal
        returns (uint256 value)
    {
        ...
        value = bridgeToken == address(0) ? messagingFee.nativeFee + sendParam.amountLD : messagingFee.nativeFee;
    ...
    function lzCompose(
        ...
        OftYieldConfig memory oftYieldConfig = oftLookup[bridgeToken];
        if (!oftYieldConfig.permissioned) revert OnlyPermissionedOft();
        if (address(IOAppCore(_from).endpoint()) != msg.sender) revert OnlyLzEndpoint();

        bytes memory composeMsg = OFTComposeMsgCodec.composeMsg(_message);
        // note amount is in shares
        uint256 amountShares = OFTComposeMsgCodec.amountLD(_message);
        uint256 amountAssets = IYieldOft(bridgeToken).sharesToAssets(amountShares);

        emit ReceivedOft(bridgeToken, amountAssets);
        ...
        // redeem shares for underlying token
        if (oftYieldConfig.l1ChainId == block.chainid) {
            _redeemUnderlying(oftYieldConfig, amountShares, bridgeToken, postBridge);
        } else {
            _updateSwapInstructions(amountAssets, bridgeToken, postBridge);
        }
     ...
     */
    function _getSendParam(BridgeCall calldata bridgeCall, address bridgeToken, address oft)
        private
        view
        returns (SendParam memory sendParam)
    {
        ...
        // convert to shares
        uint256 amountShares = IYieldOft(bridgeToken).assetsToShares(bridgeCall.amount);
        uint256 amount = (amountShares / rate) * rate;
    ...
    function _redeemUnderlying(
        OftYieldConfig memory oftYieldConfig,
        uint256 amount,
        address bridgeToken,
        SwapInstructions memory postBridge
    ) private {
        uint256 redeemAmount = IERC4626(IL1OftRouter(oftYieldConfig.l1Router).vault()).previewRedeem(amount);
        IYieldOft(bridgeToken).approveShares(oftYieldConfig.l1Router, amount);
        IL1OftRouter(oftYieldConfig.l1Router).redeem(amount, address(this));
        _updateSwapInstructions(redeemAmount, oftYieldConfig.underlying, postBridge);
    }

    function _updateSwapInstructions(uint256 amount, address tokenIn, SwapInstructions memory postBridge)
        private
        pure
    {
        if (postBridge.swapParams.tokenIn != tokenIn) {
            postBridge.swapParams.tokenIn = tokenIn;
        }

        if (postBridge.swapParams.amountIn != amount) {
