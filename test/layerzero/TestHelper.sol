...
import { SendUln302Mock as SendUln302 } from "@layerzerolabs/lz-evm-oapp-v2/test/mocks/SendUln302Mock.sol";
import { SimpleMessageLibMock } from "@layerzerolabs/lz-evm-oapp-v2/test/mocks/SimpleMessageLibMock.sol";
...
contract TestHelper is Test, OptionsHelper {
    ...
    function hasPendingPackets(uint32 _dstEid, bytes32 _dstAddress) public view returns (bool flag) {
        DoubleEndedQueue.Bytes32Deque storage queue = packetsQueue[_dstEid][_dstAddress];
        return queue.length() > 0;
    }

    function getNextInflightPacket(uint32 _dstEid, bytes32 _dstAddress) public view returns (bytes memory packetBytes) {
        DoubleEndedQueue.Bytes32Deque storage queue = packetsQueue[_dstEid][_dstAddress];
        if (queue.length() > 0) {
    ...
    function getNextInflightPacketGuid(uint32 _dstEid, bytes32 _dstAddress) public view returns (bytes32 guid) {
        DoubleEndedQueue.Bytes32Deque storage queue = packetsQueue[_dstEid][_dstAddress];
        if (queue.length() > 0) {
            guid = queue.back();
        }
